...
import { usePreviewStore, usePreviewStoreApi } from '../store'
import { TabBar, TAB_DND_TYPE, type TabDragItem } from './tab-bar'
  ...
  >({
    ...
    canDrop(item) {
      if (item.paneId === paneId) {
        const pane = storeApi.getState().panes[paneId]
...
export function EditorPane({ paneId }: EditorPaneProps): React.JSX.Element {
  ...
  const splitPaneWithTab = usePreviewStore((s) => s.splitPaneWithTab)
  const storeApi = usePreviewStoreApi()
  ...
  const handleFocus = useCallback(() => {
    const currentFocused = storeApi.getState().focusedPaneId
    if (currentFocused !== paneId) {
      setFocusedPane(paneId)
    }
  }, [paneId, setFocusedPane, storeApi])
  ...
  return (
    ...
    >
      ...
      >
        <PreviewEditor key={relativePath} relativePath={relativePath} />
